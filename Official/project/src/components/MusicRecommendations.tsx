...
export function MusicRecommendations({ emotion }: Props) {
  ...
  useEffect(() => {
    // Function to fetch playlist based on the current emotion
  ...
  // Return null if no emotion or playlist is available
  if (!emotion || !playlistId) return null;

  // Function to handle fetching the next playlist
  ...
  {loading ? (
    // Show loading spinner while fetching playlist
